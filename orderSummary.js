var tellimus = { 
  ...
  for(let i = 0; i<tellimus.rows.length; i++){
      const list = [tellimus.rows[i].name, tellimus.rows[i].amount, (tellimus.rows[i].price *  tellimus.rows[i].vat +  tellimus.rows[i].price) *  tellimus.rows[i].amount]
        console.log(list.join(" "))
        var kogusumma = kogusumma + ((tellimus.rows[i].price *  tellimus.rows[i].vat +  tellimus.rows[i].price) *  tellimus.rows[i].amount)
  }
